#include <stdlib.h>
#include <stdio.h>
#include <mpi.h>
#include <string.h>
#include "mmio.h"
#include "specifications.h"
#include "parallel_reading.h"


// BONUS 1: PARALLEL MATRIX READING
// skip comment lines starting with '%' in matrix market file
// parse matrix dimensions
// only called by rank 0 !
int parse_mm_header(char* buffer, char* buffer_end, char** cursor_out, int* n_rows, int* n_cols, int* nnz){
    char* cursor = buffer;

    while(cursor < buffer_end && *cursor == '%'){
        char* next = memchr(cursor, '\n', buffer_end - cursor);
        if (!next) return -1;
        cursor = next + 1;
    }

    // parse dimensions
    int args_found = sscanf(cursor, "%d %d %d", n_rows, n_cols, nnz);
    if (args_found != 3) {
        return -1;
    }

    // move past header line
    char* next = memchr(cursor, '\n', buffer_end - cursor);
    if (next) {
        cursor = next + 1;
    } else {
        return -1; // no newline after dimensions is an error
    }
    
    *cursor_out = cursor;
    return 0;
}

// adjust cursor for non-zero ranks to skip partial first line
void skip_partial_line(char* buffer, MPI_Offset bytes_read, char** cursor_out){
    if (bytes_read > 0) {
        char* eol = memchr(buffer, '\n', bytes_read);
        if (eol != NULL) {
            *cursor_out = eol + 1;
        } else {
            *cursor_out = buffer + bytes_read;
        }
    }
}

// grow dynamic array (safe realloc pattern ?)
int grow_array(void** array, unsigned* capacity, size_t elem_size){
    *capacity *= 2;
    void* new_ptr = realloc(*array, (*capacity) * elem_size);
    if (!new_ptr) {
        return -1;
    }
    *array = new_ptr;
    return 0;
}

// READ FILE CHUNKS
char* read_file_chunk(MPI_File fh, int rank, int comm_size, MPI_Offset file_size, MPI_Offset* bytes_read_out) {
    // calculate chunk boundaries
    MPI_Offset chunk_size = file_size / comm_size;
    MPI_Offset my_offset = rank * chunk_size;
    
    // last rank takes remainder
    if (rank == comm_size - 1) {
        chunk_size = file_size - my_offset;
    }
    
    // add padding to ensure we can complete partial lines
    int padding = 1024;
    MPI_Offset bytes_to_read = chunk_size + padding;
    if (my_offset + bytes_to_read > file_size) {
        bytes_to_read = file_size - my_offset;
    }
    
    char* buffer = (char*)malloc(bytes_to_read + 1);
    if (!buffer) return NULL;
    
    MPI_Status status;
    int ret = MPI_File_read_at_all(fh, my_offset, buffer, bytes_to_read, MPI_CHAR, &status);
    if (ret != MPI_SUCCESS) {
        free(buffer);
        return NULL;
    }
    
    buffer[bytes_to_read] = '\0';
    *bytes_read_out = bytes_to_read;
    
    return buffer;
}

// PARSE AND PARTITION
SendBuffers* create_send_buffers(int comm_size, int default_capacity) {
    SendBuffers* sb = (SendBuffers*)malloc(sizeof(SendBuffers));
    if (!sb) return NULL;
    
    sb->rows = (unsigned**)malloc(comm_size * sizeof(unsigned*));
    sb->cols = (unsigned**)malloc(comm_size * sizeof(unsigned*));
    sb->vals = (double**)malloc(comm_size * sizeof(double*));
    sb->counts = (int*)calloc(comm_size, sizeof(int));
    sb->capacities = (int*)malloc(comm_size * sizeof(int));
    
    if (!sb->rows || !sb->cols || !sb->vals || !sb->counts || !sb->capacities) {
        free(sb->rows);
        free(sb->cols);
        free(sb->vals);
        free(sb->counts);
        free(sb->capacities);
        free(sb);
        return NULL;
    }
    
    for (int i = 0; i < comm_size; i++) {
        sb->capacities[i] = default_capacity;
        sb->rows[i] = (unsigned*)malloc(default_capacity * sizeof(unsigned));
        sb->cols[i] = (unsigned*)malloc(default_capacity * sizeof(unsigned));
        sb->vals[i] = (double*)malloc(default_capacity * sizeof(double));
        
        if (!sb->rows[i] || !sb->cols[i] || !sb->vals[i]) {
            // Cleanup and return NULL
            for (int j = 0; j <= i; j++) {
                free(sb->rows[j]);
                free(sb->cols[j]);
                free(sb->vals[j]);
            }
            free(sb->rows);
            free(sb->cols);
            free(sb->vals);
            free(sb->counts);
            free(sb->capacities);
            free(sb);
            return NULL;
        }
    }
    
    return sb;
}

int add_to_send_buffer(SendBuffers* sb, int dest_rank, unsigned row, unsigned col, double val) {
    int cnt = sb->counts[dest_rank];
    int cap = sb->capacities[dest_rank];
    
    // Grow if needed
    if (cnt >= cap) {
        sb->capacities[dest_rank] *= 2;
        cap = sb->capacities[dest_rank];
        
        unsigned* new_rows = (unsigned*)realloc(sb->rows[dest_rank], cap * sizeof(unsigned));
        unsigned* new_cols = (unsigned*)realloc(sb->cols[dest_rank], cap * sizeof(unsigned));
        double* new_vals = (double*)realloc(sb->vals[dest_rank], cap * sizeof(double));
        
        if (!new_rows || !new_cols || !new_vals) {
            return -1;
        }
        
        sb->rows[dest_rank] = new_rows;
        sb->cols[dest_rank] = new_cols;
        sb->vals[dest_rank] = new_vals;
    }
    
    sb->rows[dest_rank][cnt] = row;
    sb->cols[dest_rank][cnt] = col;
    sb->vals[dest_rank][cnt] = val;
    sb->counts[dest_rank]++;
    
    return 0;
}


void free_send_buffers(SendBuffers* sb, int comm_size) {
    if (!sb) return;
    
    for (int i = 0; i < comm_size; i++) {
        free(sb->rows[i]);
        free(sb->cols[i]);
        free(sb->vals[i]);
    }
    free(sb->rows);
    free(sb->cols);
    free(sb->vals);
    free(sb->counts);
    free(sb->capacities);
    free(sb);
}

int parse_and_partition(char* buffer, MPI_Offset chunk_size, 
                               MPI_Offset bytes_read, int rank, int comm_size,
                               unsigned** local_rows, unsigned** local_cols, 
                               double** local_vals, unsigned* local_count,
                               unsigned* local_capacity, SendBuffers* send_bufs,
                               unsigned n_rows) {
    char* cursor = buffer;
    char* end_of_buffer = buffer + bytes_read;
    char* chunk_end = buffer + chunk_size;
    if (chunk_end > end_of_buffer) chunk_end = end_of_buffer;
    
    while (cursor < chunk_end) {
        char* next_newline = memchr(cursor, '\n', end_of_buffer - cursor);
        if (!next_newline) break;
        
        int curr_row, curr_col;
        double curr_val;
        int args_found = sscanf(cursor, "%d %d %lf", &curr_row, &curr_col, &curr_val);
        
        if (args_found == 3) {
            // Convert to 0-based indexing
            curr_row--;
            curr_col--;
            
            // Block distribution: contiguous rows per rank
            // Rank 0 owns rows [0, n_rows/P), Rank 1 owns [n_rows/P, 2*n_rows/P), etc.
            int destination_rank = (curr_row * comm_size) / n_rows;
            if (destination_rank >= comm_size) {
                destination_rank = comm_size - 1;  // Safety: last rank gets remainder
            }
            
            if (destination_rank == rank) {
                // Store locally
                if (*local_count >= *local_capacity) {
                    // Grow all three arrays together
                    unsigned new_capacity = (*local_capacity) * 2;
                    
                    unsigned* new_rows = (unsigned*)realloc(*local_rows, new_capacity * sizeof(unsigned));
                    unsigned* new_cols = (unsigned*)realloc(*local_cols, new_capacity * sizeof(unsigned));
                    double* new_vals = (double*)realloc(*local_vals, new_capacity * sizeof(double));
                    
                    if (!new_rows || !new_cols || !new_vals) {
                        // Don't free on realloc failure - original pointers still valid
                        return -1;
                    }
                    
                    *local_rows = new_rows;
                    *local_cols = new_cols;
                    *local_vals = new_vals;
                    *local_capacity = new_capacity;
                }
                
                (*local_rows)[*local_count] = curr_row;
                (*local_cols)[*local_count] = curr_col;
                (*local_vals)[*local_count] = curr_val;
                (*local_count)++;
            } else {
                // Add to send buffer
                if (add_to_send_buffer(send_bufs, destination_rank, 
                                      curr_row, curr_col, curr_val) != 0) {
                    return -1;
                }
            }
        }
        
        cursor = next_newline + 1;
    }
    
    return 0;
}

int pack_send_data(SendBuffers* sb, int comm_size,
                         unsigned** packed_rows, unsigned** packed_cols, 
                         double** packed_vals, int** sdispls, int* total_send) {
    *sdispls = (int*)malloc(comm_size * sizeof(int));
    if (!*sdispls) return -1;
    
    *total_send = 0;
    for (int i = 0; i < comm_size; i++) {
        (*sdispls)[i] = *total_send;
        *total_send += sb->counts[i];
    }
    
    *packed_rows = (unsigned*)malloc(*total_send * sizeof(unsigned));
    *packed_cols = (unsigned*)malloc(*total_send * sizeof(unsigned));
    *packed_vals = (double*)malloc(*total_send * sizeof(double));
    
    if (!*packed_rows || !*packed_cols || !*packed_vals) {
        free(*sdispls);
        free(*packed_rows);
        free(*packed_cols);
        free(*packed_vals);
        return -1;
    }
    
    for (int r = 0; r < comm_size; r++) {
        memcpy(&(*packed_rows)[(*sdispls)[r]], sb->rows[r], 
               sb->counts[r] * sizeof(unsigned));
        memcpy(&(*packed_cols)[(*sdispls)[r]], sb->cols[r], 
               sb->counts[r] * sizeof(unsigned));
        memcpy(&(*packed_vals)[(*sdispls)[r]], sb->vals[r], 
               sb->counts[r] * sizeof(double));
    }
    
    return 0;
}

int exchange_data(int* send_counts, int comm_size,
                        unsigned* packed_s_rows, unsigned* packed_s_cols, 
                        double* packed_s_vals, int* sdispls,
                        unsigned** recv_rows, unsigned** recv_cols, 
                        double** recv_vals, int* total_recv) {
    int local_error = 0;
    int global_error = 0;
    
    // Initialize output pointers to NULL
    *recv_rows = NULL;
    *recv_cols = NULL;
    *recv_vals = NULL;
    *total_recv = 0;
    
    // Exchange counts
    int* recv_counts = (int*)malloc(comm_size * sizeof(int));
    if (!recv_counts) {
        local_error = 1;
    }
    
    // Check if any rank failed BEFORE MPI_Alltoall
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        free(recv_counts);
        return -1;
    }
    
    MPI_Alltoall(send_counts, 1, MPI_INT, recv_counts, 1, MPI_INT, MPI_COMM_WORLD);
    
    // Calculate receive displacements
    int* rdispls = (int*)malloc(comm_size * sizeof(int));
    if (!rdispls) {
        local_error = 1;
    }
    
    // Check if any rank failed BEFORE continuing
    local_error = (rdispls == NULL) ? 1 : 0;
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        free(recv_counts);
        free(rdispls);
        return -1;
    }
    
    *total_recv = 0;
    for (int i = 0; i < comm_size; i++) {
        rdispls[i] = *total_recv;
        *total_recv += recv_counts[i];
    }
    
    // For huge matrices, check if total_recv is reasonable
    // Max safe allocation: 1 GB per rank (adjust if needed)
    long max_elements_per_type = (1024LL * 1024LL * 1024LL) / 8; // 1GB / 8 bytes (double)
    if (*total_recv > max_elements_per_type) {
        local_error = 1; // Mark as error if too large
    } else {
        local_error = 0;
    }
    
    // Check if any rank has data that's too large
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        if (*total_recv > max_elements_per_type) {
            fprintf(stderr, "[Rank] Warning: total_recv=%d exceeds safe limit, may cause memory issues\n", *total_recv);
        }
        free(recv_counts);
        free(rdispls);
        return -1;
    }
    
    // Allocate receive buffers
    *recv_rows = (unsigned*)malloc(*total_recv * sizeof(unsigned));
    *recv_cols = (unsigned*)malloc(*total_recv * sizeof(unsigned));
    *recv_vals = (double*)malloc(*total_recv * sizeof(double));
    
    if (!*recv_rows || !*recv_cols || !*recv_vals) {
        local_error = 1;
    } else {
        local_error = 0;
    }
    
    // Check if any rank failed BEFORE MPI_Alltoallv
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        free(recv_counts);
        free(rdispls);
        free(*recv_rows);
        free(*recv_cols);
        free(*recv_vals);
        *recv_rows = NULL;
        *recv_cols = NULL;
        *recv_vals = NULL;
        return -1;
    }
    
    // Execute exchanges - all ranks guaranteed to reach here together
    int mpi_ret = MPI_Alltoallv(packed_s_rows, send_counts, sdispls, MPI_UNSIGNED,
                  *recv_rows, recv_counts, rdispls, MPI_UNSIGNED, MPI_COMM_WORLD);
    if (mpi_ret != MPI_SUCCESS) {
        free(recv_counts);
        free(rdispls);
        free(*recv_rows);
        free(*recv_cols);
        free(*recv_vals);
        return -1;
    }
    
    mpi_ret = MPI_Alltoallv(packed_s_cols, send_counts, sdispls, MPI_UNSIGNED,
                  *recv_cols, recv_counts, rdispls, MPI_UNSIGNED, MPI_COMM_WORLD);
    if (mpi_ret != MPI_SUCCESS) {
        free(recv_counts);
        free(rdispls);
        free(*recv_rows);
        free(*recv_cols);
        free(*recv_vals);
        return -1;
    }
    
    mpi_ret = MPI_Alltoallv(packed_s_vals, send_counts, sdispls, MPI_DOUBLE,
                  *recv_vals, recv_counts, rdispls, MPI_DOUBLE, MPI_COMM_WORLD);
    if (mpi_ret != MPI_SUCCESS) {
        free(recv_counts);
        free(rdispls);
        free(*recv_rows);
        free(*recv_cols);
        free(*recv_vals);
        return -1;
    }
    
    free(recv_counts);
    free(rdispls);
    return 0;
}

// MERGE DATA
int merge_data(unsigned** local_rows, unsigned** local_cols, 
                     double** local_vals, unsigned local_count,
                     unsigned* recv_rows, unsigned* recv_cols, 
                     double* recv_vals, int total_recv,
                     unsigned* final_nnz) {
    *final_nnz = local_count + total_recv;
    
    unsigned* final_rows = (unsigned*)realloc(*local_rows, *final_nnz * sizeof(unsigned));
    unsigned* final_cols = (unsigned*)realloc(*local_cols, *final_nnz * sizeof(unsigned));
    double* final_vals = (double*)realloc(*local_vals, *final_nnz * sizeof(double));
    
    if (!final_rows || !final_cols || !final_vals) {
        return -1;
    }
    
    *local_rows = final_rows;
    *local_cols = final_cols;
    *local_vals = final_vals;
    
    // Append received data
    memcpy(&(*local_rows)[local_count], recv_rows, total_recv * sizeof(unsigned));
    memcpy(&(*local_cols)[local_count], recv_cols, total_recv * sizeof(unsigned));
    memcpy(&(*local_vals)[local_count], recv_vals, total_recv * sizeof(double));
    
    return 0;
}

// MAIN FUNCTION
int load_mm_chunked(const char* filename, Sparse_Coordinate* matrix, 
                    int rank, int comm_size) {
    MPI_File fh;
    int ret;
    int local_error = 0;
    int global_error = 0;
    
    // open file collectively
    ret = MPI_File_open(MPI_COMM_WORLD, filename, MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);
    if (ret != MPI_SUCCESS) {
        fprintf(stderr, "[Rank %d] Error opening file: %s\n", rank, filename);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    
    MPI_Offset file_size;
    MPI_File_get_size(fh, &file_size);
    
    // PHASE 1: read file chunks
    MPI_Offset bytes_read;
    char* buffer = read_file_chunk(fh, rank, comm_size, file_size, &bytes_read);
    if (!buffer) {
        fprintf(stderr, "[Rank %d] Failed to read file chunk\n", rank);
        MPI_File_close(&fh);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    
    MPI_Offset chunk_size = file_size / comm_size;
    MPI_Offset my_offset = rank * chunk_size;
    if (rank == comm_size - 1) {
        chunk_size = file_size - my_offset;
    }
    
    // parse header, adjust cursor
    char* cursor = buffer;
    char* end_of_buffer = buffer + bytes_read;
    int n_rows = 0, n_cols = 0, nnz = 0;
    
    if (rank == 0) {
        if (parse_mm_header(buffer, end_of_buffer, &cursor, 
                           &n_rows, &n_cols, &nnz) != 0) {
            fprintf(stderr, "[Rank 0] Error parsing matrix header\n");
            free(buffer);
            MPI_File_close(&fh);
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }
    } else {
        skip_partial_line(buffer, bytes_read, &cursor);
    }
    
    MPI_Bcast(&n_rows, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&n_cols, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&nnz, 1, MPI_INT, 0, MPI_COMM_WORLD);
    
    // Better initial capacity estimate (30% overhead for safety)
    unsigned local_capacity = (unsigned)((nnz / comm_size) * 1.3) + 1000;
    unsigned local_count = 0;
    unsigned* local_rows = (unsigned*)malloc(local_capacity * sizeof(unsigned));
    unsigned* local_cols = (unsigned*)malloc(local_capacity * sizeof(unsigned));
    double* local_vals = (double*)malloc(local_capacity * sizeof(double));

    if (!local_rows || !local_cols || !local_vals) {
        local_error = 1;
    }
    
    // Check if any rank failed
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        if (rank == 0) fprintf(stderr, "[Rank %d] %d rank(s) failed to allocate local arrays\n", rank, global_error);
        free(buffer);
        free(local_rows);
        free(local_cols);
        free(local_vals);
        MPI_File_close(&fh);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }

    // PHASE 2: parse and partition
    SendBuffers* send_bufs = create_send_buffers(comm_size, 1000);
    
    // Check if any rank failed
    local_error = (send_bufs == NULL) ? 1 : 0;
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        if (rank == 0) fprintf(stderr, "[Rank %d] %d rank(s) failed to create send buffers\n", rank, global_error);
        free_send_buffers(send_bufs, comm_size);
        free(local_rows);
        free(local_cols);
        free(local_vals);
        free(buffer);
        MPI_File_close(&fh);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    
    // Adjust cursor position in buffer
    MPI_Offset cursor_offset = cursor - buffer;
    MPI_Offset bytes_remaining = bytes_read - cursor_offset;

    // FIXED: For rank 0, adjust chunk_size to account for header
    MPI_Offset effective_chunk_size = chunk_size;
    if (rank == 0) {
        effective_chunk_size = chunk_size - cursor_offset;
    }

    ret = parse_and_partition(cursor, effective_chunk_size, 
                             bytes_remaining, rank, comm_size,
                             &local_rows, &local_cols, &local_vals, 
                             &local_count, &local_capacity, send_bufs, n_rows);
    
    // Log parsing statistics for debugging huge matrices
    if (rank == 0) {
        fprintf(stderr, "[Rank %d] Parsed header: %d rows, %d cols, %d nnz\n", 
                rank, n_rows, n_cols, nnz);
    }
    MPI_Barrier(MPI_COMM_WORLD); // Sync before logging
    fprintf(stderr, "[Rank %d] Parsed local: %u elements, send_bufs totals: %d elements\n", 
            rank, local_count, send_bufs->counts[0] + send_bufs->counts[1]); // simplified
    
    free(buffer);
    
    local_error = (ret != 0) ? 1 : 0;
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        if (rank == 0) fprintf(stderr, "[Rank %d] %d rank(s) failed during parse_and_partition\n", rank, global_error);
        free_send_buffers(send_bufs, comm_size);
        free(local_rows);
        free(local_cols);
        free(local_vals);
        MPI_File_close(&fh);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    
    // PHASE 3: Exchange data
    unsigned* packed_s_rows = NULL;
    unsigned* packed_s_cols = NULL;
    double* packed_s_vals = NULL;
    int* sdispls = NULL;
    int total_send = 0;
    
    ret = pack_send_data(send_bufs, comm_size, &packed_s_rows, &packed_s_cols, 
                        &packed_s_vals, &sdispls, &total_send);
    
    local_error = (ret != 0) ? 1 : 0;
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        if (rank == 0) fprintf(stderr, "[Rank %d] %d rank(s) failed during pack_send_data\n", rank, global_error);
        free_send_buffers(send_bufs, comm_size);
        free(local_rows);
        free(local_cols);
        free(local_vals);
        free(packed_s_rows);
        free(packed_s_cols);
        free(packed_s_vals);
        free(sdispls);
        MPI_File_close(&fh);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    
    unsigned* recv_rows = NULL;
    unsigned* recv_cols = NULL;
    double* recv_vals = NULL;
    int total_recv = 0;
    
    ret = exchange_data(send_bufs->counts, comm_size, 
                       packed_s_rows, packed_s_cols, packed_s_vals, sdispls,
                       &recv_rows, &recv_cols, &recv_vals, &total_recv);
    
    free_send_buffers(send_bufs, comm_size);
    free(packed_s_rows);
    free(packed_s_cols);
    free(packed_s_vals);
    free(sdispls);
    
    local_error = (ret != 0) ? 1 : 0;
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        if (rank == 0) fprintf(stderr, "[Rank %d] %d rank(s) failed during exchange_data\n", rank, global_error);
        free(local_rows);
        free(local_cols);
        free(local_vals);
        free(recv_rows);
        free(recv_cols);
        free(recv_vals);
        MPI_File_close(&fh);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    
    // PHASE 4: merge 
    unsigned final_nnz = 0;
    ret = merge_data(&local_rows, &local_cols, &local_vals, local_count,
                    recv_rows, recv_cols, recv_vals, total_recv, &final_nnz);
    
    free(recv_rows);
    free(recv_cols);
    free(recv_vals);
    
    local_error = (ret != 0) ? 1 : 0;
    MPI_Allreduce(&local_error, &global_error, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if (global_error > 0) {
        if (rank == 0) fprintf(stderr, "[Rank %d] %d rank(s) failed during merge_data\n", rank, global_error);
        free(local_rows);
        free(local_cols);
        free(local_vals);
        MPI_File_close(&fh);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    
    // populate output
    matrix->n_rows = n_rows;
    matrix->n_cols = n_cols;
    matrix->nnz = final_nnz;
    matrix->row_indices = local_rows;
    matrix->col_indices = local_cols;
    matrix->values = local_vals;
    
    MPI_File_close(&fh);
    return 0;
}